...
const africastalkingsms = (message) => {
  const credentials = {
    apiKey: process.env.AFRICA_API_KEY,
    username: process.env.AFRICA_API_USERNAME,
  };
  ...
  // Use the service
  const options = {
    to: process.env.AFRICA_API_MESSAGE_TO,
    ...
    .catch((error) => {
      ...
      exports.forgotPassword = asyncHandler(async (req, res, next) => {
        ...
        try {
          ...
          return res.status(200).json({ success: true, data: "sms sent" });
        } catch (err) {
          console.log(err);
          user.resetPasswordToken = undefined;
          user.resetPasswordExpire = undefined;
      ...
      const africastalkingsms = (message) => {
        ...
        // Use the service
        const options = {
          to: ["+234xxxxxxxxxx", "+234xxxxxxxxxx"], // enter phone numbers
          from: process.env.AFRICA_API_MESSAGE_FROM,
          message: message,
        };
